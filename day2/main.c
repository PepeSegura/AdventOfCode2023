...
/*   Updated: 2023/12/02 14:01:39 by psegura-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	check_data(int r, int g, int b)
{
	if (r > 12 || g > 13 || b > 14)
		return (0);
	return (1);
...
int	locate_data(char **matrix)
{
	...
	while (matrix[i])
	{
		...
		trim_matrix(out);
		red		= find_color(out, "red");
		green	= find_color(out, "green");
		blue	= find_color(out, "blue");
		ft_free_matrix(out);
		if (check_data(red, green, blue) == 0)
			return (0);
		i++;
	}
	return (1);
